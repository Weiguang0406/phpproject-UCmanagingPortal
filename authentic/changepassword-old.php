...
//echo "<script> alert('submit'+$u2)</script>";
    ...
    if (($_SERVER["REQUEST_METHOD"] == "POST") && (isset($_POST['submit']))) {
        ...
        echo "<script> alert('22222222222222222222221')</script>";
        //$u1 = $_SERVER["REQUEST_METHOD"];
        $u2 = $_POST['old_password'];

       // echo "<script> alert('$u1')</script>";
        echo "<script> alert('$u2')</script>";
        //validation username
        if (empty(trim($_POST["old_password"]))) {
            $old_password_err = "Please enter old_password";
        } else if (validation_password($header_username,$u2) === false) {
        ...
        if ((empty($old_password_err)) && (empty($new_password_err)) && (empty($confirm_password_err))) {
            ...
            switch ($roles) {
                case 'teacher':
                    ...
                    echo "<script>window.location.href='../teachers/teacher_login.php'</script>";

                    break;
                case 'student':
                  //  header("location:student_login.php");
                    echo "<script>window.location.href='../students/student_login.php'</script>";

                    break;
                case 'admin':
                    echo "<script>window.location.href='../admins/admin_login.php'</script>";
